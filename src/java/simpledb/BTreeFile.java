...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		switch(pid.pgcateg()){
			...
			case BTreePageId.INTERNAL:
				...
				//要查找的时空key则寻找该B+Tree做左边的叶节点
				if(f==null){
					return findLeafPage(tid, dirtypages, bip.getChildId(0), perm, f);
				}
				//在key所在子节点中寻找key
				Iterator<BTreeEntry> bipIterator = bip.iterator();
				BTreeEntry entry=null;
				while(bipIterator.hasNext()){
					entry=bipIterator.next();
			...
			default:
				throw new IllegalStateException("Unexpected value: " + pid.pgcateg());
		}
	}
	 ...
	 * leaf node with permission perm.
	 *
	 * If f is null, it finds the right-most leaf page -- used for the reverse iterator
	 *
	 * @param tid - the transaction id
	 * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
	 * @param pid - the current page being searched
	 ...
	 *
	 */
	private BTreeLeafPage findLeafPageR(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
										Field f)
			throws DbException, TransactionAbortedException {
		// some code goes here
		switch(pid.pgcateg()){
			...
			case BTreePageId.INTERNAL:
				//获得此内部页
				BTreeInternalPage bip= (BTreeInternalPage) this.getPage(tid,
						dirtypages,
						pid,
						Permissions.READ_ONLY);
				//要查找的时空key则寻找该B+Tree做右边的叶节点
				if(f==null){
					return findLeafPage(tid, dirtypages,
							bip.reverseIterator().next().getRightChild(),
							perm, f);
				}
				//在key所在子节点中寻找key
				Iterator<BTreeEntry> bipIterator = bip.reverseIterator();
				BTreeEntry entry=null;
				while(bipIterator.hasNext()){
					entry=bipIterator.next();
	...
	/**
	 * Convenience method to find a leaf page when there is no dirtypages HashMap.
	 * Used by the BTreeFile reverseIterator.
	 * @see #findLeafPage(TransactionId, HashMap, BTreePageId, Permissions, Field)
	 *
	 * @param tid - the transaction id
	 * @param pid - the current page being searched
	 ...
	 *
	 */
	BTreeLeafPage findLeafPageR(TransactionId tid, BTreePageId pid, Permissions perm,
								Field f)
			throws DbException, TransactionAbortedException {
		return findLeafPageR(tid, new HashMap<PageId, Page>(), pid, perm, f);
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreeLeafPage rightPage=(BTreeLeafPage)getEmptyPage(tid
				...
				,BTreePageId.LEAF);
		//移动一半元组
		Iterator<Tuple> ts=page.reverseIterator();
		...
		Field middleKeyField=rightPage.getTuple(0).getField(this.keyField);
		BTreeInternalPage parentPage=this.getParentWithEmptySlots(tid
				,dirtypages
				,page.getParentId()
				,middleKeyField);
		parentPage.insertEntry(new BTreeEntry(middleKeyField
				,page.getId()
				,rightPage.getId()));
		//updata parent pointer
		this.updateParentPointers(tid,dirtypages,parentPage);
		//update the sibling pointers
		BTreePageId rightRightPageId=page.getRightSiblingId();
		if(rightRightPageId!=null) {
			BTreeLeafPage rightRightPage= (BTreeLeafPage) getPage(tid
					,dirtypages
					,rightRightPageId
					,Permissions.READ_WRITE);
			rightRightPage.setLeftSiblingId(rightPage.getId());
		}
		rightPage.setRightSiblingId(rightRightPageId);
		rightPage.setLeftSiblingId(page.getId());
		page.setRightSiblingId(rightPage.getId());
		//Return the page
		if(field.compare(Op.GREATER_THAN_OR_EQ,middleKeyField)){
			return rightPage;
		}else{
			return page;
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//NOTE:在过样例时出现simpledb.BTreeRootPtrPage cannot be cast to simpledb.HeapPage,需要改为更抽象的类

		//获得新页
		BTreeInternalPage rightPage=(BTreeInternalPage) getEmptyPage(tid
				,dirtypages
				,BTreePageId.INTERNAL);
		//移动一半元组
		Iterator<BTreeEntry> es=page.reverseIterator();
		...
		BTreeEntry middleEntry=es.next();
		page.deleteKeyAndRightChild(middleEntry);
		BTreeInternalPage parentPage=this.getParentWithEmptySlots(tid
				,dirtypages
				,page.getParentId()
				,middleEntry.getKey());
		parentPage.insertEntry(new BTreeEntry(middleEntry.getKey()
				,page.getId()
				,rightPage.getId()));
		//updata parent pointer
		this.updateParentPointers(tid
				,dirtypages
				,parentPage);
		this.updateParentPointers(tid
				,dirtypages
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//NOTE:测试样例中要求两边平衡，只移动一个不行
		Iterator<Tuple> moveds;
		if(isRightSibling){
			moveds=sibling.iterator();
		}else{
			moveds=sibling.reverseIterator();
		}
		int numMoved=(sibling.getNumTuples()-page.getNumTuples())/2;
		Tuple moved=null;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry> moveds=leftSibling.reverseIterator();
		int numMoved=(leftSibling.getNumEntries()-page.getNumEntries())/2-1;
		for(int i=0;i<numMoved;i++){
			BTreeEntry deleted=moveds.next();
			leftSibling.deleteKeyAndRightChild(deleted);
			page.insertEntry(deleted);
		}

		BTreeEntry deleted=moveds.next();
		leftSibling.deleteKeyAndRightChild(deleted);
		parentEntry.setKey(deleted.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry> moveds=rightSibling.iterator();
		int numMoved=(rightSibling.getNumEntries()-page.getNumEntries())/2-1;
		for(int i=0;i<numMoved;i++){
			BTreeEntry deleted=moveds.next();
			rightSibling.deleteKeyAndLeftChild(deleted);
			page.insertEntry(deleted);
		}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int numMovedTuples= rightPage.getNumTuples();
		for(int i=0;i<numMovedTuples;i++){
			Tuple t=ts.next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int numMovedEntries= rightPage.getNumEntries();
		for(int i=0;i<numMovedEntries;i++){
			BTreeEntry e=es.next();
...
//		}
	 ...
	 * @param tid - the transaction id
	 * @param ipred - the index predicate value to filter on
	 * @return an reverse iterator for the filtered tuples
	 */
	...
	/**
	 * Get an iterator for all tuples in this B+ tree file in reverse sorted order. This method
	 * will acquire a read lock on the affected pages of the file, and may block until
	 * the lock can be acquired.
	 *
	 * @param tid - the transaction id
...
class BTreeFileReverseIterator extends AbstractDbFileIterator {
	...
	/**
	 * Constructor for this reverseIterator
	 * @param f - the BTreeFile containing the tuples
	 * @param tid - the transaction id
	 */
	...
	/**
	 * Open this iterator by getting an reverseIterator on the first leaf page
	 */
	public void open() throws DbException, TransactionAbortedException {
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, BTreeRootPtrPage.getId(f.getId()), Permissions.READ_ONLY);
		BTreePageId root = rootPtr.getRootId();
		//TODO:
		curp = f.findLeafPageR(tid, root, Permissions.READ_ONLY, null);
...
/**
 * Helper class that implements the DbFileIterator for search tuples in reverse order on a
 * B+ Tree File
 */
class BTreeSearchReverseIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		...
		if(ipred.getOp() == Op.EQUALS || ipred.getOp() == Op.GREATER_THAN
				|| ipred.getOp() == Op.GREATER_THAN_OR_EQ) {
			curp = f.findLeafPageR(tid, root, Permissions.READ_ONLY, ipred.getField());
		}
		else {
			curp = f.findLeafPageR(tid, root, Permissions.READ_ONLY, null);
		}
